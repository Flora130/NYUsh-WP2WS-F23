...
function setupThree() {
  ...
  ring1 = getRing(WORLD_HALF / 1, WORLD_HALF / 1.2)
  ring1.position.y = -100

  ring2 = getRing(WORLD_HALF / 4, WORLD_HALF / 1.5)
  ...
  tLight.setPosition(0, 400, 0);
  lights.push(tLight);

  pole = getBox();
  pole.scale.set(10, 500, 10);
  scene.add(pole);
...
function updateThree() {
  if (volume > 0.1) {
    let v = map(volume, 0.1, 1, 0, 500);
    pole.scale.y = 480 + v;
  } else {
    pole.scale.y = 480
  }
  //pole.geometry.attributes.scale.needsUpdate = true;
  ...
  let posArray2 = ring3.geometry.attributes.position.array;
  ...
  // let x = cos(frame * 0.02) * WORLD_HALF / 5;
  // let z = sin(frame * 0.02) * WORLD_HALF / 5;
  // for (let i = 0; i < 10; i++) {
  //   particles.push(new Particle(x, 200, z, 10));
  // }
...
function getLight() {
  const light = new THREE.SpotLight(0xFFFFFF, 5, 1000, PI / 3, 0.5, 0.1); // (color, intensity, distance (0=infinite), angle, penumbra, decay)
...
class Particle {
  constructor(x, y, z, size) {
    ...
    this.mesh.scale.set(this.size, this.size, this.size1);
  }
  move() {
  ...
  bump() {
    if (volume > 0.1) {
      let height = map(volume, 0, 2, 1, 50, 100);
      this.size1 += height;
    }
    // if (volume > 0.1) {
...
class Light {
  ...
  move() {
    ...
    let radialDistance = WORLD_HALF / 6;
